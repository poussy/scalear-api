...
class SharedItemsControllerTest < ActionDispatch::IntegrationTest
 ...
 test "should show accepted shared items for user" do
  ...
  test "should not create shared_item with wrong email" do
    shared_item_count = SharedItem.count
    post '/en/shared_items', params: {shared_item:{data: {modules:[2,1],lectures:[3],quizzes:[1],customlinks:[5]}},shared_with: "tototwashere@gmail.com"}, headers: @headers, as: :json
    assert_response 422
    resp =  JSON.parse(response.body)
    assert_equal resp['errors'] , "Teacher doesn't exist"
    assert_equal SharedItem.count , shared_item_count  
  end

  test "should destroy shared_item" do
    shared_item_count = SharedItem.count
    url = '/en/shared_items/1'
    delete url ,params: {},headers: @headers

    resp =  JSON.parse(response.body)
    assert_equal SharedItem.count , shared_item_count - 1 
  end
