...
namespace :db do
	...
	task :import_csv_course, [:url] => :environment do |t, args|
		if args[:url].include? "https://drive.google.com"
				args[:url].gsub!(/file\/d\/|open\?id\=/ , "uc?export=view&id=")
		end
	 	p args[:url]
		...
		Zip::ZipInputStream::open(tmp_file.path) do |io|
			while (entry = io.get_next_entry)
				...
				if model_name == "Back" || model_name == "Pause"
					model_name = "VideoEvent"
				end
				if model_name != "Discussion" && model_name != "LectureQuestion"
		...
		new_course = Course.new tables["Course"].first.slice(*Course.attribute_names - ["id"])
		new_course.save(:validate => false)

		tables["Group"].each do |old_group|
			new_group = Group.new old_group.slice(*Group.attribute_names - ["id"])
			...
			ActiveRecord::Base.transaction do
				tables["Lecture"].select{|l| l["group_id"] == old_group["id"]}.each do |old_lecture|
					new_lecture = Lecture.new old_lecture.slice(*Lecture.attribute_names - ["id"])
					...
					tables["Event"].select{|l| l["lecture_id"] == old_lecture["id"] && (l["quiz_id"] == nil || l["quiz_id"].blank? || l["quiz_id"]=="") }.each do |old_event|
						new_event = Event.new old_event.slice(*Event.attribute_names - ["id"])
					...
					tables["OnlineQuiz"].select{|q| q["lecture_id"] == old_lecture["id"]}.each do |old_online_quiz|
						new_online_quiz = OnlineQuiz.new old_online_quiz.slice(*OnlineQuiz.attribute_names - ["id"])
						...
						tables["OnlineAnswer"].select{|a| a["online_quiz_id"] == old_online_quiz["id"]}.each do |old_online_answer|
							new_online_answer = OnlineAnswer.new old_online_answer.slice(*OnlineAnswer.attribute_names - ["id"])
				...
				tables["Quiz"].select{|quiz| quiz["group_id"] == old_group["id"]}.each do |old_quiz|
					new_quiz = Quiz.new old_quiz.slice(*Quiz.attribute_names - ["id"])
					...
					tables["Question"].select{|q| q["quiz_id"] == old_quiz["id"]}.each do |old_question|
						new_question = Question.new old_question.slice(*Question.attribute_names - ["id"])
						...
						tables["Answer"].select{|a| a["question_id"] == old_question["id"]}.each do |old_answer|
							new_answer = Answer.new old_answer.slice(*Answer.attribute_names - ["id"])
						...
						tables["FreeAnswer"].select{|a| a["question_id"] == old_question["id"]}.each do |old_free_answer|
							new_free_answer = FreeAnswer.new old_free_answer.slice(*FreeAnswer.attribute_names - ["id"])
			...
			tables["CustomLink"].select{|l| l["group_id"] == old_group["id"] }.each do |old_custom_link|
				new_custom_link = CustomLink.new old_custom_link.slice(*CustomLink.attribute_names - ["id"])
		...
		new_course.user = User.find_by_email(tables["TeacherEnrollment"].first["email"])
