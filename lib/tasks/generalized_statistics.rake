namespace :db do
    ...
    task :generalized_statistics, [:domain,:year]=> [:environment] do |t, args|
        ...
        (1..53).each do |w|
            start_day+=1.week
            if start_day < Date.new(args[:year].to_i+1)
                weeks<<start_day
            end    
        end      
        activity_per_week=[]
        weeks.each_with_index{|someDay,i| activity_per_week.push(generate_activity_statistics(args[:domain],weeks[i],weeks[i+1])) }              
    ...
    def send_statistics(activity_per_week,year,domain)
        ...
        csv_files[:activty_statistics]= CSV.generate do |statistic|
            statistic << ["startAt",'active_teachers','active_students','active_courses']
        ...
        file_name = domain+"_activity_"+year.to_s+".zip"
    ...
    def generate_activity_statistics(domain ,startAt ,endAt)
        if !startAt.nil? & !endAt.nil? & !domain.nil?
            ...
            createdAt_last_year_query = "created_at between '#{startAt}' and '#{endAt}'"
            updatedAt_last_year_query = "updated_at between '#{startAt}' and '#{endAt}'"
            domain_users = "users.email like '%#{domain}%'"
            
            new_users = User.where(createdAt_last_year_query).where(domain_users)
            ...
            updated_users = User.where(updatedAt_last_year_query).where(id_in,domain_users_ids)
            updated_courses = Course.where(updatedAt_last_year_query).where(user_id_in,domain_users_ids)
            lec_views = LectureView.where(updatedAt_last_year_query).where(user_id_in,domain_users_ids)
            quiz_solved = QuizGrade.where(updatedAt_last_year_query).where(user_id_in,domain_users_ids)
            updated_note= VideoNote.where(updatedAt_last_year_query).where(user_id_in,domain_users_ids)
            updated_confused= Confused.where(updatedAt_last_year_query).where(user_id_in,domain_users_ids)
            
            updated_modules = Group.where(updatedAt_last_year_query).where(course_id_in,domain_courses_ids)
            updated_lectures= Lecture.where(updatedAt_last_year_query).where(course_id_in,domain_courses_ids)
            updated_quizzes = Quiz.where(updatedAt_last_year_query).where(course_id_in,domain_courses_ids)
            updated_links = CustomLink.where(updatedAt_last_year_query).where(course_id_in,domain_courses_ids)
            announcements = Announcement.where(updatedAt_last_year_query).where(course_id_in,domain_courses_ids)
            ...
            return [startAt,active_teachers,active_students,active_courses]
        end
    end
end